...
function SmokeyBackground({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // It's crucial to use the program before setting uniforms
      gl.useProgram(program);
      ...
      gl.uniform2f(
        iMouseLocation,
        isHoveringRef.current ? mousePositionRef.current.x : width / 2,
        isHoveringRef.current ? height - mousePositionRef.current.y : height / 2
      );
